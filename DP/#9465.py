...
결과 : 각 테스트 케이스 마다, 2n개의 스티커 중에서 두 변을 공유하지 않는 스티커 점수의 최댓값을 출력한다.
...
for i in range(test_num):
    num = int(input())
    arr = [[0 for i in range(2)] for i in range(num+1)]
    ...
    dp = [[0 for i in range(2)] for i in range(num+2)]
    ...
    for k in range(2, num+1):
        for j in range(2):
            if j == 0:
                dp[k][j] = max(dp[k-2][j+1], dp[k-1][j+1]) + arr[k][j]
            elif j == 1:
                dp[k][j] = max(dp[k-2][j-1], dp[k-1][j-1]) + arr[k][j]
