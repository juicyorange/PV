...
input_command_count = int(input())

stack1 = input_string
stack2 = []

for i in range(input_command_count):
    ...
    if(input_command[0] == "L"):
        ...
        if(stack2):
            stack1.append(stack2.pop(len(stack2)-1))
    elif(input_command[0] == "B"):
        if(stack1):
            stack1.pop()
    elif(input_command[0] == "P"):
        # insert, remove는 굉장히 시간소요가 큰 함수이다. 다른방법을 고안해야한다.
        # input_string.insert(now_cursor, input_command[1])

        stack1.append(input_command[1])

result = stack1 + list(reversed(stack2))
